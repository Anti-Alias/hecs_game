...
use std::sync::Arc;
use super::{Asset, AssetId, AssetLoader, AssetPath, AssetStorage, AssetStorageMut, DynLoader, DynStorage, Handle, InnerAssetStorage, PathHash, Protocol};
...
impl AssetManager {
    ...
    pub fn try_fast_load<A: Asset>(&mut self, path: &str, path_hash: PathHash) -> Result<Handle<A>, LoadError> {
        ...
        std::thread::spawn(move || {
            ...
            let dyn_asset = match loader.dyn_load(&path, &bytes) {
...
mod test {
    ...
    use crate::plugins::asset2::*;

    struct YmlLoader;
    impl AssetLoader for YmlLoader {
        type AssetType = Value;
        fn load(&self, _path: &AssetPath, bytes: &[u8]) -> anyhow::Result<Self::AssetType> {
